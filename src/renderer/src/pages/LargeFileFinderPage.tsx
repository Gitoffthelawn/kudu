...
export function LargeFileFinderPage() {
  ...
  const handleScan = async () => {
    ...
    try {
      const result = await window.kudu?.largeFilesScan?.({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const result = await window.kudu?.largeFilesDelete?.(paths, store.deleteMode)
      if (result) {
              ...
              >
                {store.result.files.map((file, idx) => (
                  ...
                  >
                    ...
                    />
                    <FileUp className="h-4 w-4 shrink-0" style={{ color: '#8a8a96' }} strokeWidth={1.5} />
                    <span
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation()
                        window.kudu?.largeFilesOpenLocation?.(file.path)
                      }}
...
function StatCard({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    ...
    >
      ...
      <div className="mt-1 text-[18px] font-bold" style={{ color: accent ? '#f59e0b' : '#fafafa' }}>{value}</div>
    </div>
  )
}
...
function EmptyState({ title, description }: { title: string; description: string }) {
  return (
    <div className="flex flex-1 flex-col items-center justify-center py-20 text-center">
      <FileUp className="mb-4 h-12 w-12" style={{ color: '#3f3f46' }} strokeWidth={1.2} />
