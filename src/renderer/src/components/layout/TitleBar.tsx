  ...
  return (
    <div className="drag-region flex h-9 items-center justify-between border-b border-zinc-800/50 bg-bg-base px-4">
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm font-medium text-zinc-400">Kudu</span>
      </div>

      <div className="no-drag flex items-center">
        <WindowButton onClick={() => window.kudu.windowMinimize()} aria-label="Minimize">
          <Minus className="h-3.5 w-3.5" />
        </WindowButton>
        <WindowButton onClick={() => window.kudu.windowMaximize()} aria-label="Maximize">
